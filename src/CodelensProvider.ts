...
export class CodelensProvider implements vscode.CodeLensProvider {
  ...
  constructor() {
    // Khởi tạo regex để tìm các chuỗi i18n dạng "XXXX.XXXX...." (có ít nhất một dấu chấm)
    // Tìm các chuỗi trong quotes có chứa ít nhất một dấu chấm
    // Pattern: "word.word" hoặc 'word.word' hoặc `word.word`
    this.regex =
      /(['"`])([a-zA-Z_$][a-zA-Z0-9_$]*(?:\.[a-zA-Z_$][a-zA-Z0-9_$]*)+)\1/gs;
    // Đọc setting thư mục i18n
    this.i18nFolders = vscode.workspace
      .getConfiguration("codelens-i18n")
      .get<string[]>("i18nFolder", ["i18n"]);
    // Đọc setting ngôn ngữ hiển thị
    this.displayLanguage = vscode.workspace
      .getConfiguration("codelens-i18n")
      .get<string>("displayLanguage", "ja");
  ...
  public resolveCodeLens(
    ...
    if (
      ...
      for (const preferredLocale of uniquePriorityLanguages) {
        ...
        for (const locale of this.i18nCache.keys()) {
          const localeWithoutPrefix = locale.includes(".")
            ? locale.split(".").pop()
            : locale;
      ...
      if (
        translationsWithValues.length === 0 &&
        translationsWithoutValues.length === 0
      ) {
      ...
      codeLens.command = {
        ...
        arguments: [
          i18nKey,
          [...translationsWithValues, ...translationsWithoutValues],
          localeFilePaths,
        ], // Truyền key, translations và file paths
      };
      return codeLens;
    }
